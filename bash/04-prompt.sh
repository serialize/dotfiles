...
function _ps1_format_git() {
	local name=$GIT_CURRENT_BRANCH \
			mod=$GIT_CURRENT_COUNT_MOD \
			new=$GIT_CURRENT_COUNT_NEW \
			del=$GIT_CURRENT_COUNT_DEL
	[[ "$name" ]] || return 1
	
	local color="$PS1_COLOR_GIT_OK"	
	[[ "$mod" == 0 ]] || color="$PS1_COLOR_GIT_CHG" 
	mod="$(ps1_render_color $color 0) $mod"
	
	color="$PS1_COLOR_GIT_OK"	
	[[ "$new" == 0 ]] || color="$PS1_COLOR_GIT_CHG"
	$new="$(ps1_render_color $color 0) $new"
		
	color="$PS1_COLOR_GIT_OK"	
	[[ "$del" == 0 ]] || color="$PS1_COLOR_GIT_CHG" 
	del="$(ps1_render_color $color 0) $del"
...
function _ps1_render_spacer() {
	local color="$1" bgcolor="$2"  
	
	[[ "$content" ]] || return 1
	color=$(ps1_render_color "$color" "$bgcolor")
	
	echo -en "$color$PS1_SPACER"
}
function _ps1_render_block() {
	local $content="$1" color="$2" bgcolor="$3"  
	local result=
	
	[[ "$content" ]] || return 1
	color=$(ps1_render_color "$color" "$bgcolor")
	
	echo -en "$color$content"
}
function _ps1_render_block_wraped() {
	local $content="$1" color="$2" bgcolor="$3"  
	local result=
...
function ps1_render_color() {
	local color=
	if [[ "$1" ]];then
		color+="$PS1_COLOR_FORE_FORMAT"
		color+="$1"
	fi 
	if [[ "$2" ]];then
		[[ "$color" ]] && color+=";"
		color+="$PS1_COLOR_BACK_FORMAT"
		color+="$2" 
	fi
	echo -en "\033["$color"m"
}
